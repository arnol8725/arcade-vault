...
## Addendum de implementación (2026-09-12)

Durante `/spec-impl-game` se detectó que este spec fue redactado antes de que la arquitectura real de motor/canvas de Arcade Vault quedara fijada (ver `AGENTS.md` raíz y `components/games/serpentina-canvas.tsx` como referencia viva). Con confirmación explícita del usuario, la implementación **difiere del texto original** en estos puntos — las reglas de juego (grilla, entidades, colisiones, puntaje, rondas) se mantienen intactas, solo cambia dónde vive el código:

- **No** se crea `components/games/FroggerGame.tsx` monolítico. **Sí** se crea `lib/games/frogger-engine.ts` (motor framework-agnóstico, factory `createFroggerEngine(canvas, callbacks)` → `{ start, stop, setPaused, reset, setKeyState }`) + `components/games/frogger-canvas.tsx` (componente cliente delgado, patrón `callbacksRef`), igual que `rocas`/`bloque-buster`/`arkanoide`/`serpentina`.
- **No** se crea `app/games/frogger/play/page.tsx`. **Sí** se reutiliza la ruta genérica `/juego/[id]/jugar` (`app/juego/[id]/jugar/page.tsx`), que ya renderiza `GamePlayer` sin código por-juego.
- El wiring (HUD, pausa, modal de game over, guardado de score) se agrega como una rama `isFrogger` más en `components/game-player.tsx`, reutilizando el modal y el guardado ya genéricos — no se construye un modal ni un guardado de score propios.
- Razón: mantiene compatibilidad con la Fase 5 de `/spec-impl-game` (`skin-designer` y `mobile-porter` exigen explícitamente el patrón `lib/games/<slug>-engine.ts` + `components/games/<slug>-canvas.tsx`) y con el resto del catálogo ya implementado.
